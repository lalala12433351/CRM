...
export const AddLeadPage: React.FC<AddLeadViewProps> = ({
  leads,
  agents,
  customFields,
  activeAgent,
  ...
  const validateForm = () => {
    ...
    // Phone validation (max and exactly 10 digits)
    const phoneRes = validatePhone(rawPhone, true);
    if (!phoneRes.isValid) {
      errs.phone = phoneRes.error || 'Phone number must contain exactly 10 digits.';
    } else {
      const digitsOnly = phoneRes.cleanedValue;
      const last10 = digitsOnly.slice(-10);
      const existingMatch = leads.find((l) => (l.phone || '').replace(/\D/g, '').slice(-10) === last10);
      if (existingMatch) {
        errs.phone = `A lead with this phone number already exists: "${existingMatch.name}" (${existingMatch.phone}). Cannot create duplicate lead.`;
      }
    }
    ...
    if (!emailRes.isValid) {
      errs.email = emailRes.error || 'Please enter a valid email address.';
    }

    // Deal Value validation (currency limits)
    const dealRes = validateCurrencyOrNumber(dealValue, { label: 'Deal Value', minValue: 0, maxValue: 1000000000 }, false);
    if (!dealRes.isValid) {
      errs.dealValue = dealRes.error || 'Invalid deal value amount.';
    }
    ...
    // Validate Dynamic Custom Fields (required, minLength, maxLength, minValue, maxValue)
    dynamicCustomFields.forEach((field) => {
      const val = customFieldValues[field.name];
      const fieldRes = validateField(field, val);
      if (!fieldRes.isValid) {
        errs[field.name] = fieldRes.error || `${field.label} is invalid.`;
      }
    });
...
export const AddLeadView = AddLeadPage;
