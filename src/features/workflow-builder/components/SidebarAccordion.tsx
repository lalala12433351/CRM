...
export const SidebarAccordion: React.FC<SidebarAccordionProps> = ({ onItemClick, className = '' }) => {
  ...
  return (
    <div className={`w-80 flex flex-col bg-white border-r border-slate-200/90 select-none h-full shadow-2xs font-sans ${className}`}>
      {/* Sidebar Header (Without Heading Icon) */}
      <div className="p-3.5 border-b border-slate-200/90 space-y-2.5">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-8 pr-3 py-1.5 text-xs font-medium rounded-md border border-slate-200/90 bg-slate-50 text-slate-800 placeholder-slate-400 focus:outline-none focus:bg-white focus:border-[#3a2088] shadow-2xs"
          />
          {searchQuery && (
            <button
              type="button"
      ...
      {/* Accordion Categories List (Reduced Curves) */}
      <div className="flex-1 overflow-y-auto p-2.5 space-y-2.5 custom-scrollbar">
        {WORKFLOW_CATEGORIES.map((cat) => {
          ...
          return (
            <div
              key={cat.id}
              className="border border-slate-200/90 rounded-lg overflow-hidden bg-white shadow-2xs transition-all duration-150"
            >
              {/* Category Accordion Header (Without Icon) */}
              <button
                type="button"
                  ...
                  {items.length === 0 ? (
                    ...
                    items.map((item) => (
                      <div
                        ...
                        className="group flex items-start gap-2.5 p-2.5 rounded-lg border border-slate-200/90 hover:border-[#3a2088] bg-white hover:shadow-2xs cursor-grab active:cursor-grabbing transition-all select-none"
                      >
                        <div className="text-slate-300 group-hover:text-[#3a2088] pt-0.5 transition-colors">
                          <GripVertical className="w-3.5 h-3.5" />
                        </div>

                        <div className={`p-1.5 rounded-md border ${styling.border} ${styling.bg} ${styling.text} shrink-0`}>
      ...
      {/* Footer Info Box */}
      <div className="p-2.5 border-t border-slate-200/90 bg-white">
