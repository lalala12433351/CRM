  ...
  const renderDrawerContent = () => (
    <div className={`flex-1 w-full h-full bg-[#fafafa] flex flex-col overflow-hidden relative ${isEmbedded ? 'rounded-xl border border-slate-200/90 shadow-2xs' : 'shadow-2xl border-l border-slate-200'}`}>
      ...
      <div className="flex-1 overflow-y-auto ios-scroll">
        ...
        {/* Reusable Modal: Schedule Follow-Up Date & Time */}
        <ScheduleFollowUpModal
          isOpen={showFollowUpScheduler}
          onClose={() => setShowFollowUpScheduler(false)}
          lead={lead}
          targetStage={pendingTargetStatus}
          onConfirm={({ lead: confirmedLead, targetStage, combinedDate, remarks }) => {
            const updatedLead: Lead = {
              ...confirmedLead,
              status: targetStage as LeadStatus,
              followUpAt: combinedDate,
              notes: remarks ? `${confirmedLead.notes ? confirmedLead.notes + '\n' : ''}[Follow-up Remark]: ${remarks}` : confirmedLead.notes,
              updatedAt: new Date().toISOString()
            };
