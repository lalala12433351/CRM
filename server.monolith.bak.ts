import "dotenv/config"; // Load .env file into process.env (local dev)
...
async function startServer() {
  ...
  app.get('/api/auth/meta/callback', async (req: any, res: any) => {
    ...
    const clientId = 'default_admin';

    try {
      const reqProto = req.headers['x-forwarded-proto'] || req.protocol || 'https';
      const reqHost = req.get('host') || '';
      const dynamicCallback = `${reqProto}://${reqHost}/api/auth/meta/callback`;
      const redirectUriToUse = (reqHost.includes('ngrok') || reqHost.includes('loca.lt') || reqHost.includes('localhost'))
        ? dynamicCallback
        : 'https://d3pcv3wpcxqhl2.cloudfront.net/api/auth/meta/callback';

      const tokenRes = await axios.get('https://graph.facebook.com/v22.0/oauth/access_token', {
        params: {
          client_id: '1785911265462186',
          client_secret: '2499233acf77fa2ce858d5a56a90c04a',
